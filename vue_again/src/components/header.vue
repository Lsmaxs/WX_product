	...
	<header v-if="$route.name!='index'">
      ...
      <img src="https://o4j806krb.qnssl.com/public/images/cnodejs_light.svg" alt="cnodejs-logo">
		...
		<ul class="navbar">
			...
			<li v-if="!token"><a v-link="{name: 'login'}">登入{{$route.router}}</a></li>
      <template v-else>
        <li><a v-link="{name: 'messages'}">未读消息<span class="h"  v-if="msgCount">{{ msgCount }}</span></a></li>
        <li><a v-link="{name: 'photo'}">发照片<span class="h"  v-if="msgCount">{{ msgCount }}</span></a></li>
        <li><select name="searchClass"><option value="1">小一(222)班</option><option value="2">小一(999)班</option></select></li>
...
<script>
  ...
  export default {
    ...
    ready() {
      ...
      if (false&&document.cookie.length > 0) {
