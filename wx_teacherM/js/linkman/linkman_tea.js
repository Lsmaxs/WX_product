...
define(function(require, exports, module) {
	...
	function init(){
		...
		$("body").on("click",".cls-sendword",function(){
			MtaH5.clickStat('jxhd_linkman_tea_words');
	...
	function loadLocalData(){
		...
		try{
			...
			$.post(common.ctx+"/wap/tea/linkman/tea/list",
					{
						userId:userId,
						cropId:cropId
					},
			...
			$.post(common.ctx+"/wap/tea/linkman/par/list",
					{
						userId:userId,
						cropId:cropId
					},
	 ...
	 * @returns {String}
	 */
	function genTeaLinkman(deptList){
		var tmpStr = '';
		if(deptList.length > 0){
			tmpStr = '<h2>教师通讯录</h2>';
			for(var j=0;j<deptList.length;j++){
				tmpStr += '<div class="address_list"><div class="tit" >'+deptList[j].deptName+'<span class="dy_some">&nbsp;</span></div><ul>';
				var teaLinkmanList = deptList[j].linkmanTeaList;
				if(teaLinkmanList){
...
//						}
						...
						tmpStr += '<b>'+teaLinkmanList[i].name+'</b><br />&nbsp;&nbsp;&nbsp;'+//teaLinkmanList[i].phone+
						 		  '<span class="address_cont">'+
						 		  //'<a href="javascript:;" data-classCode="'+teaLinkmanList[i].classCode+'" data-userId="'+teaLinkmanList[i].uuid+'" ><img src="'+ctx+'/static/wap/images/ico_xx.png" /></a>'+
						 		  '<a href="tel:'+teaLinkmanList[i].phone+'"><img src="../../images/ico_phone.png" /></a>'+
						 		  '</span></li>';
					}
				}
				tmpStr += '</ul></div>';
			...
			for(var i=0;i<classLinkmanList.length;i++){
				...
				for(var j=0;j<linkmanParList.length;j++){
					if(j == linkmanParList.length - 1){
						tmpStr += '<li class="none_line line_h21" >';
					}else{
						tmpStr += '<li class="line_h21" >';
					}
//					if(!linkmanParList[j].icon || linkmanParList[j].icon == '' || linkmanParList[j].icon == 'null'){
//						tmpStr += '<span class="follow_ico"></span>';
//					}
					if(linkmanParList[j].isFocuson == 1){
					...
					tmpStr += '<b>'+linkmanParList[j].stuName+'家长'+'</b> <em>'+linkmanParList[j].name+'</em><br />'+ linkmanParList[j].phone+
		  			  		  '<span class="address_cont">'+
		  			  		  '<a class="cls-sendword" href="javascript:;" data-classCode="'+classLinkmanList[i].classCode+'" data-userId="'+linkmanParList[j].uuid+'" data-userName="'+linkmanParList[j].name+'" ><img src="../../images/ico_xx.png" /></a>'+
		  			  		  '<a href="tel:'+linkmanParList[j].phone+'"><img src="../../images/ico_phone.png" /></a>'+
		  			  		  '</span></li>';
				}		  
				tmpStr += '</ul></div>';
			}
	...
	function sLinkmanOnBlur(){
		...
		if(queryParam){
			...
			for(var i=0;i<teaObjList.length;i++){
				...
				if(teaObjList[i].linkmanTeaList){
					for(var j=0;j<teaObjList[i].linkmanTeaList.length;j++){
						if(teaObjList[i].linkmanTeaList[j].name && teaObjList[i].linkmanTeaList[j].name.indexOf(queryParam) >= 0){
							teaArr.push(teaObjList[i].linkmanTeaList[j]);
						}
					}
				}
				if(teaArr.length > 0){
					deptArr.push({deptName:teaObjList[i].deptName,linkmanTeaList:teaArr});
				}
			}
			for(var i=0;i<classObjList.length;i++){
				...
				if(parArr.length > 0){
					var clazzInfo = {className:classObjList[i].className,classCode:classObjList[i].classCode,linkmanParList:parArr};
					classArr.push(clazzInfo);
				}
			}
			var tmpStr = '';
			if(deptArr.length > 0){
				tmpStr += genTeaLinkman(deptArr);
			}
	...
	function sendWords(classCode,targetUuid,targetName){
		var cropId = $("#cropId").val();
		var userId = $("#userId").val();
		document.location.href = common.ctx+"/wap/html/words/words_tea_edit.html?classCode="+classCode+"&targetUuid="+targetUuid+"&targetName="+encodeURI(targetName)+"&cropId="+cropId+"&userId="+userId+"&funcType=0&funUniq=";
	}
