...
define(function(require, exports, module) {
	...
	function loadLocalData(){
		...
		try{
			...
			$.post(common.ctx+"/wap/tea/words/list/1",
					{
						userId:userId,
						cropId:cropId,
						type:type
			...
			if(globalVal.pickUpFlag){
				...
				if(($(document).height()-range) <= totalheight) {
					if(globalVal.page_flag == 0){ 
						...
						var cropId = $("#cropId").val();
						var type = $("#type").val();
						$.post(common.ctx+"/wap/tea/words/list/"+(globalVal.page+1),
								{
									userId:userId,
									cropId:cropId,
									type:type
		...
		if(wordsList && wordsList.length){
			for(var i=0;i<wordsList.length;i++){
				tmpStr += '<div class="list_box" >'+
						  '<input type="hidden" class="noticeNu" value="'+wordsList[i].nu+'"  />'+
					 	  '<div class="list_from">';
				...
				if(type==2){
					...
					tmpStr += '<span class="face_pic"><img src="'+wordsList[i].targetIcon+'" onerror="javascript:this.src=\'../../images/user_pace.png\'" /></span>'+wordsList[i].targetName;
				}
			    if(wordsList[i].targetType == 1){
				...
				tmpStr += '<div class="time fl tr">'+wordsList[i].createTime+'</div></div>'+
						  ...
						  if(wordsList[i].imageUrls){
							  var picClass = "";
							  if(wordsList[i].imageUrls.length == 1){
								  picClass = "list_pic_y";
							  }else if(wordsList[i].imageUrls.length == 4){
								  picClass = "list_pic_4";
							  }else{
							  ...
							  if(wordsList[i].imageUrls.length == 1){
								  tmpStr += '<a href="javascript:;"><span><img onload="imgPreDeal(this);" src="'+wordsList[i].imageUrls[0]+'" onerror="javascript:this.src=\'../../images/no_photo.png\'" /></span></a>';
							  }else{
								  for(var j=0;j<wordsList[i].imageUrls.length;j++){
									  tmpStr += '<a href="javascript:;"><span><img onload="winFixImg(this);" src="'+wordsList[i].imageUrls[j]+'" onerror="javascript:this.src=\'../../images/no_photo.png\'" /></span></a>';
								  }
							  }
							  tmpStr += '</div>';
						  }
				...
				tmpStr += '</div>'+
						  ...
						  if(wordsList[i].replys){
							  for(var z=0;z<wordsList[i].replys.length;z++){
								  if(wordsList[i].replys[z].iSaid == 1){
									  tmpStr += '<p><img src="'+wordsList[i].replys[z].icon+'" onerror="javascript:this.src=\'../../images/user_pace.png\'" />';
									  tmpStr += wordsList[i].replys[z].content+'<br /><b>'+wordsList[i].replys[z].createTime+'</b></p>';
								  }else{
									  tmpStr += '<p><img src="'+wordsList[i].replys[z].icon+'" onerror="javascript:this.src=\'../../images/user_pace.png\'" />';
									  tmpStr += wordsList[i].replys[z].content+'<br /><b>'+wordsList[i].replys[z].createTime+'</b></p>';
								  }
	...
	function goWordsList(type){
		var cropId = $("#cropId").val();
		var userId = $("#userId").val();
		document.location.href = common.ctx+"/wap/html/words/words_tea_list.html?cropId="+cropId+"&userId="+userId+"&type="+type;
	}
	...
	function goWordsDetail(nu){
		var cropId = $("#cropId").val();
		var userId = $("#userId").val();
		document.location.href = common.ctx+"/wap/html/words/words_tea_detail.html?cropId="+cropId+"&userId="+userId+"&nu="+nu;
	}
